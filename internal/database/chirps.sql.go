...
const getChirpsWithOptions = `-- name: GetChirpsWithOptions :many
...
type GetChirpsWithOptionsParams struct {
	UserID  uuid.UUID
	Column2 interface{}
}

func (q *Queries) GetChirpsWithOptions(ctx context.Context, arg GetChirpsWithOptionsParams) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsWithOptions, arg.UserID, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
